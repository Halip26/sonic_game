...
screen_size_display = (width_screen, height_screen) = (900, 455)
FPS = 60
gravity = 0.6
...
def introduction_screen():
    ado_sonic = sonic(44, 47)
    ado_sonic.blinking = True
    starting_game = False
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE or event.key == pygame.K_UP:
                        ado_sonic.jumping = True
                        ado_sonic.blinking = False
                        ado_sonic.movement[1] = -1 * ado_sonic.jumpSpeed
...
def gameplay():
    ...
    while not g_exit:
        ...
        while not g_over:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_Sonic.rect.bottom == int(0.98 * height_screen):
                                gamer_Sonic.jumping = True
                                if pygame.mixer.get_init() != None:
                                    jump_sound.play()
                                gamer_Sonic.movement[1] = -1 * gamer_Sonic.jumpSpeed

                        if event.key == pygame.K_DOWN:
                            if not (gamer_Sonic.jumping and gamer_Sonic.dead):
                                gamer_Sonic.ducking = True

                    if event.type == pygame.KEYUP:
                        if event.key == pygame.K_DOWN:
                            gamer_Sonic.ducking = False

            for c in cactusan:
                c.movement[0] = -1 * gp
                if pygame.sprite.collide_mask(gamer_Sonic, c):
                    gamer_Sonic.dead = True
                    if pygame.mixer.get_init() != None:
                        die_sound.play()
            ...
            skyClouds.update()
            new_grnd.update()
            score_boards.update(gamer_Sonic.score)
            ...
            if gamer_Sonic.dead:
                g_over = True
                if gamer_Sonic.score > highest_scores:
                    highest_scores = gamer_Sonic = gamer_Sonic.score
